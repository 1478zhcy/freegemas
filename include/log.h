...
#define LOC()                                                                  \
  string(__FUNCTION__) + " :(" + __FILE__ + ":" + std::to_string(__LINE__) +   \
      ")  "
 ...
 *
 * Utiliza sintaxis de flujo para facilitar el uso. Existen cuatro niveles de
 * log, aunque por ahora no se diferencian en nada más que en el nombre.
 *
...
public:
   ...
   * Escrito por conveniencia para unificar el logging de constructores y
   * destructores.
   *
  ...
  /**
   * @brief Vuelca el flujo con los mensajes de log en el destino indicado, por
   * ahora la salida estándar.
   */
  ~Log();
...
protected:
  /// Flujo en el que se vuelcan los mensajes antes de ser enviados al destino
  /// final.
