 ...
 *
 * El funcionamiento es sencillo. Creamos un objeto Animación al que le
indicamos el número de parámetros que nos interesaría animar. Por ejemplo, para
la posición de un objeto, lo usual es animar la posición X y la posición Y.
Además, al constructor de la animación le indicamos la duración de la animación,
la ecuación a seguir (según queramos imitar un movimiento de aceleración o
deceleración) y la espera inicial antes de comenzar la animación. \code
 Animacion miAnimacion (2, 30, Animacion::tEaseInQuad, 0);
 \endcode

 Seguidamente, por cada parámetro tendremos que indicar cuál es el valor inicial
y final de la animación: \code
 ...
 Ahora, para que la animación vaya avanzando habrá que ir llamando al método
#update; el método #get nos servirá para leer el valor de cada parámetro y
aplicarlo a nuestro programa. \code miAnimacion . update(); cout << "Posición
horizontal: " << miAnimacion.get(0) << "\n"
      << "Posición vertical: " << miAnimacion.get(1) << endl;
\endcode

 * Contiene una serie de ecuaciones con las que lograr efectos de aceleración y
deceleración en las animaciones.
 *
 *
 *
