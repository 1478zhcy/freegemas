...
class FloatingScore {
public:
  FloatingScore(GoSDL::Window *parentWindow, int score, float x, float y,
                float z)
    ...
    // Build the image
    mScoreImage =
        tempFont.renderText(std::to_string(score), {255, 255, 255, 255});
    mScoreImageShadow =
        tempFont.renderText(std::to_string(score), {0, 0, 0, 255});
  }

  bool ended() { return mCurrentStep == mTotalSteps; }
  ...
  void draw() {
    ...
    mScoreImageShadow.draw(posX + 2, posY + 2, z_ - 0.1, 1, 1, 0,
                           (int)(p * 255));
