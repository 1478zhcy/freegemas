...
GoSDL::Window::Window(unsigned width, unsigned height, std::string caption,
                      bool fullscreen, double updateInterval)
    : mWidth(width), mHeight(height), mCaption(caption),
      mFullscreen(fullscreen), mUpdateInterval(updateInterval) {
  // Get starting ticks
  ...
  // Create window
  mWindow = SDL_CreateWindow(mCaption.c_str(), SDL_WINDOWPOS_UNDEFINED,
...
void GoSDL::Window::show() {
  ...
  while (mShouldRun) {
    ...
    for (qIt = mDrawingQueue.begin(); qIt != mDrawingQueue.end(); ++qIt) {
      ...
      // Set coloring
      SDL_SetTextureColorMod(op->mTexture, op->mColor.r, op->mColor.g,
                             op->mColor.b);

      // Draw the texture
      int res =
          SDL_RenderCopyEx(this->mRenderer, op->mTexture, NULL, &(op->mDstRect),
                           op->mAngle, NULL, SDL_FLIP_NONE);
...
void GoSDL::Window::close() { mShouldRun = false; }

void GoSDL::Window::enqueueDraw(SDL_Texture *texture, SDL_Rect destRect,
                                double angle, float z, Uint8 alpha,
                                SDL_Color color) {
