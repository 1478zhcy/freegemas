...
ScoreTable::ScoreTable(Game *p, int score, string gameMode) : mGame(p) {
  ...
  // Cache-render texts
  mRenderedHeader = fntH1.renderTextWithShadow(
      _("GAME OVER"), {255, 255, 255, 255}, 1, 3, {0, 0, 0, 128});
  mRenderedScore = fntLcdBig.renderTextWithShadow(
      std::to_string(score), {255, 255, 255, 255}, 1, 3, {0, 0, 0, 128});
  mRenderedLastScore = fntH2.renderTextWithShadow(
      _("Latest high score: ") + std::to_string(lastScore),
      {255, 255, 255, 255}, 1, 3, {0, 0, 0, 128});
}
...
void ScoreTable::draw(int x, int y, int z) {
  ...
  // Draw the score and its shadow
  mRenderedLastScore.draw(center - mRenderedLastScore.getWidth() / 2,
                          y + 67 + 80, z);
}
