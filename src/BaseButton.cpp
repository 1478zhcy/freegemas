...
void BaseButton::set(GoSDL::Window *parentWindow, std::string caption,
                     std::string iconPath) {
...
void BaseButton::setText(std::string caption) {
  ...
  mImgCaption = textFont.renderTextWithShadow(caption, {255, 255, 255, 255}, 1,
                                              2, {0, 0, 0, 128});

  // Calculate the position of the text
  if (mHasIcon) {
    mTextHorizontalPosition =
        40 + (mImgBackground.getWidth() - 40) / 2 - mImgCaption.getWidth() / 2;
  }

  else {
    mTextHorizontalPosition =
        mImgBackground.getWidth() / 2 - mImgCaption.getWidth() / 2;
  }
}
...
bool BaseButton::clicked(unsigned int mX, unsigned int mY) {
  if (mX > mLastX && mX < mLastX + mImgBackground.getWidth() && mY > mLastY &&
      mY < mLastY + mImgBackground.getHeight()) {
    return true;
  } else {
    return false;
  }
