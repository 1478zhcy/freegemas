...
void Board::generate() {
  ...
  // lDEBUG << "The generated Board.has no direct matches but some possible
  // solutions.";
}
...
void Board::del(int x, int y) { squares[x][y] = sqEmpty; }
...
void Board::calcFallMovements() {
  ...
  for (int x = 0; x < 8; ++x) {
    ...
    for (int y = 7; y >= 0; --y) {
      ...
      // If the current square is empty, every square above it should fall one
      // position
    ...
    // We start counting from top to bottom. Once we find a square, we stop
    // counting
    for (int y = 0; y < 8; ++y) {
      if (squares[x][y] != sqEmpty)
        break;
