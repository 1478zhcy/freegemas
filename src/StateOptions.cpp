...
void StateOptions::updateButtonTexts() {
  ...
  fullscreenText +=
      std::string(mOptions.getFullscreenEnabled() ? _("On") : _("Off"));

  // Menu text items
  SDL_Color menuTextColor = {255, 255, 255, 255},
            menuShadowColor = {0, 0, 0, 128};
  renderedTexts.push_back(mFont.renderTextWithShadow(musicText, menuTextColor,
                                                     0, 2, menuShadowColor));
  renderedTexts.push_back(mFont.renderTextWithShadow(soundText, menuTextColor,
                                                     0, 2, menuShadowColor));
#ifndef __vita__
  renderedTexts.push_back(mFont.renderTextWithShadow(
      fullscreenText, menuTextColor, 0, 2, menuShadowColor));
#endif
  renderedTexts.push_back(mFont.renderTextWithShadow(_("Back"), menuTextColor,
                                                     0, 2, menuShadowColor));
...
StateOptions::~StateOptions() { lDEBUG << Log::DES("StateOptions"); }
