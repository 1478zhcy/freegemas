...
void GameBoard::endGame(int score) {
  ...
  scoreTable =
      std::make_shared<ScoreTable>(mGame, score, mGame->getCurrentState());
}
...
void GameBoard::update() {
  ...
  mFloatingScores.erase(remove_if(mFloatingScores.begin(),
                                  mFloatingScores.end(),
                                  std::bind<bool>(&FloatingScore::ended, _1)),
                        mFloatingScores.end());

  // Remove the hiden particle systems
  mParticleSet.erase(remove_if(mParticleSet.begin(), mParticleSet.end(),
...
void GameBoard::draw() {
  ...
  std::for_each(mFloatingScores.begin(), mFloatingScores.end(),
                std::bind(&FloatingScore::draw, _1));

  // Draw each particle system
  std::for_each(mParticleSet.begin(), mParticleSet.end(),
  ...
  // On to the gem drawing procedure. Let's have a pointer to the image of each
  // gem
    ...
    for (int j = 0; j < 8; ++j) {
      ...
      // When two correct gems have been selected, they switch positions
        ...
        if (mSelectedSquareFirst.equals(i, j)) {
          imgX = Animacion::easeOutQuad(
              float(mAnimationCurrentStep), float(posX + i * 65),
              float((mSelectedSquareSecond.x - mSelectedSquareFirst.x) * 65),
              float(mAnimationShortTotalSteps));

          imgY = Animacion::easeOutQuad(
              float(mAnimationCurrentStep), float(posY + j * 65),
      ...
      // When the two selected gems have switched, the matched gems disappear
        ...
        if (mGroupedSquares.matched(Coord(i, j))) {
          imgAlpha = 255 * (1 - (float)mAnimationCurrentStep /
                                    mAnimationShortTotalSteps);
        }
      }
      ...
      // When there are no more matching movements, the board disappears
        ...
        imgY =
            Animacion::easeInQuad(float(mAnimationCurrentStep),
                                  float(posY + mBoard.squares[i][j].origY * 65),
                                  float(mBoard.squares[i][j].destY * 65),
                                  float(mAnimationLongTotalSteps));
      }
...
void GameBoard::createFloatingScores() {
  ...
  for (Match &m : mGroupedSquares) {
    ...
    mFloatingScores.emplace_back(
        FloatingScore(mGame, score, m.midSquare().x, m.midSquare().y, 80));
