...
bool GoSDL::Image::setWindowAndPath(GoSDL::Window *parentWindow,
                                    std::string path) {
  mParentWindow = parentWindow;
  mPath = getBasePath() + path;
  return loadTexture();
...
bool GoSDL::Image::loadTexture() {
  ...
  SDL_Texture *texture =
      IMG_LoadTexture(mParentWindow->getRenderer(), mPath.c_str());
...
int GoSDL::Image::getWidth() { return mWidth; }

int GoSDL::Image::getHeight() { return mHeight; }

bool GoSDL::Image::draw(int x, int y, int z, double factorX, double factorY,
                        float angle, Uint8 alpha, SDL_Color color) {
