...
StateHowtoplay::StateHowtoplay(Game *p) : State(p) {
  ...
  mImgTitle = fontTitle.renderTextWithShadow(
      _("How to play"), {255, 255, 255, 255}, 1, 2, {0, 0, 0, 128});
  ...
  mImgSubtitle = fontSubtitle.renderTextWithShadow(
      subtitleText.c_str(), {255, 255, 255, 255}, 1, 2, {0, 0, 0, 128});
  ...
  bodyText +=
      _("The objective of the game is to swap one gem with an adjacent gem to "
        "form a horizontal or vertical chain of three or more gems.");
  bodyText += "\n\n";
  bodyText += _("Click the first gem and then click the gem you want to swap "
                "it with. If the movement is correct, they will swap and the "
                "chained gems will disappear.");
  bodyText += "\n\n";
  bodyText += _("Bonus points are given when more than three identical gems "
                "are formed. Sometimes chain reactions, called cascades, are "
                "triggered, where chains are formed by the falling gems. "
                "Cascades are awarded with bonus points.");

  mImgBodyText = fontText.renderBlockWithShadow(
      bodyText.c_str(), {255, 255, 255, 255}, 450, 1, 2, {0, 0, 0, 128});
}
...
StateHowtoplay::~StateHowtoplay() { lDEBUG << Log::DES("StateHowtoPlay"); }
