...
void GameHint::draw() {
  ...
  if (mAnimationCurrentStep++ == mAnimationTotalSteps) {
    ...
    // Get the location
    float pX1 = 241 + mHintLocation.x * 65 -
                mImgSelector.getWidth() * (2 - p1) / 2 + 65 / 2;
    float pY1 = 41 + mHintLocation.y * 65 -
                mImgSelector.getHeight() * (2 - p1) / 2 + 65 / 2;

    // Draw the hint
    mImgSelector.draw(pX1, pY1, 10, 2 - p1, 2 - p1, 0, p1 * 255,
                      {0, 255, 0, 255});
  }
}