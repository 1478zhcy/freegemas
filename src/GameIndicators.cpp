...
GameIndicators::GameIndicators() : mGame(NULL), mStateGame(NULL) {}
...
void GameIndicators::loadResources() {
  ...
  mImgScoreHeader = tempHeaderFont.renderTextWithShadow(
      _("score"), {160, 169, 255, 255}, 1, 1, {0, 0, 0, 128});

  mImgTimeHeader = tempHeaderFont.renderTextWithShadow(
      _("time left"), {160, 169, 255, 255}, 1, 1, {0, 0, 0, 128});
...
int GameIndicators::getScore() { return mScore; }
...
void GameIndicators::regenerateScoreTexture() {
  ...
  if (mScore != mScorePrevious) {
    mImgScore =
        mFontScore.renderText(std::to_string(mScore), {78, 193, 190, 255});
...
void GameIndicators::draw() {
  ...
  mImgScoreHeader.draw(17 + mImgScoreBackground.getWidth() / 2 -
                           mImgScoreHeader.getWidth() / 2,
                       84, 3);
  ...
  if (mTimeEnabled) {
    ...
    mImgTimeHeader.draw(17 + mImgTimeBackground.getWidth() / 2 -
                            mImgTimeHeader.getWidth() / 2,
                        190, 3);
...
void GameIndicators::disableTime() { mTimeEnabled = false; }

void GameIndicators::enableTime() { mTimeEnabled = true; }