...
StateMainMenu::StateMainMenu(Game *p) : State(p) {
  ...
  // Menu target states
  mMenuTargets = {"stateGameTimetrial", "stateGameEndless", "stateHowtoplay",
                  "stateOptions", "stateQuit"};

  // Menu text items
  SDL_Color menuTextColor = {255, 255, 255, 255},
            menuShadowColor = {0, 0, 0, 128};
  mMenuRenderedTexts.push_back(mFont.renderTextWithShadow(
      _("Timetrial mode"), menuTextColor, 0, 2, menuShadowColor));
  mMenuRenderedTexts.push_back(mFont.renderTextWithShadow(
      _("Endless mode"), menuTextColor, 0, 2, menuShadowColor));
  mMenuRenderedTexts.push_back(mFont.renderTextWithShadow(
      _("How to play?"), menuTextColor, 0, 2, menuShadowColor));
  mMenuRenderedTexts.push_back(mFont.renderTextWithShadow(
      _("Options"), menuTextColor, 0, 2, menuShadowColor));
  mMenuRenderedTexts.push_back(mFont.renderTextWithShadow(
      _("Exit"), menuTextColor, 0, 2, menuShadowColor));
...
void StateMainMenu::draw() {
  ...
  int logoAlfa = clamp(
      (int)(255 * (float)mAnimationCurrentStep / mAnimationLogoSteps), 0, 255);
...
StateMainMenu::~StateMainMenu() { lDEBUG << Log::DES("StateMainMenu"); }
