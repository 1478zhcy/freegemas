...
void GoSDL::Font::setAll(Window *parentWindow, std::string path,
                         unsigned size) {
...
GoSDL::Image GoSDL::Font::renderTextWithShadow(std::string text,
                                               SDL_Color color, int shadowX,
                                               int shadowY,
                                               SDL_Color shadowColor) {

  SDL_Surface *textSurface = TTF_RenderUTF8_Blended(mFont, text.c_str(), color);
  SDL_Surface *shadowSurface =
      TTF_RenderUTF8_Blended(mFont, text.c_str(), shadowColor);
  return surfaceToImageWithShadow(textSurface, shadowSurface, shadowX, shadowY);
}

GoSDL::Image GoSDL::Font::renderBlock(std::string text, SDL_Color color,
                                      unsigned width) {

  SDL_Surface *tempSurface =
      TTF_RenderUTF8_Blended_Wrapped(mFont, text.c_str(), color, width);
  return surfaceToImage(tempSurface);
}

GoSDL::Image GoSDL::Font::renderBlockWithShadow(std::string text,
                                                SDL_Color color, unsigned width,
  ...
  SDL_Surface *textSurface =
      TTF_RenderUTF8_Blended_Wrapped(mFont, text.c_str(), color, width);
  SDL_Surface *shadowSurface =
      TTF_RenderUTF8_Blended_Wrapped(mFont, text.c_str(), shadowColor, width);
  return surfaceToImageWithShadow(textSurface, shadowSurface, shadowX, shadowY);
}

GoSDL::Image GoSDL::Font::surfaceToImage(SDL_Surface *tempSurface) {
  SDL_Texture *tempTexture =
      SDL_CreateTextureFromSurface(mParentWindow->getRenderer(), tempSurface);
...
GoSDL::Image GoSDL::Font::surfaceToImageWithShadow(SDL_Surface *textSurface,
                                                   SDL_Surface *shadowSurface,
                                                   int shadowX, int shadowY) {
  SDL_Rect rect;

  SDL_Surface *tempSurface = SDL_CreateRGBSurfaceWithFormat(
      0, shadowX + shadowSurface->w, shadowY + shadowSurface->h, 32,
